/**
 * Script to replace hardcoded pageSize values with constants
...
files.forEach(file => {
  ...
  if (hasChanges) {
    ...
    let relativePath = path
      .relative(fileDir, path.join(srcDir, "utils", "constants"))
      .replace(/\\/g, "/");
    ...
    const needsSmall =
      fileReplacements.small > 0 &&
      !originalContent.includes("PAGE_SIZE_SMALL");
    const needsMedium =
      fileReplacements.medium > 0 &&
      !originalContent.includes("PAGE_SIZE_MEDIUM");
    const needsDefault =
      fileReplacements.default > 0 &&
      !originalContent.includes("DEFAULT_PAGE_SIZE");
    ...
    const count =
      fileReplacements.small +
      fileReplacements.medium +
      fileReplacements.default;
...
console.log(`\n=== Summary ===`);
console.log(
  `Total: ${totalReplacements} replacements in ${changedFiles.length} files`
);
