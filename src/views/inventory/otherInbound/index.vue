...
import {
  approveOtherInboundOrderApi,
  createOtherInboundOrderApi,
  deleteOtherInboundOrderApi,
  getOtherInboundOrderListApi,
...
const statusOptions = Object.entries(otherInboundStatusMap).map(
  ([value, config]) => ({
    value,
    label: config.label
  })
...
const fetchData = async () => {
  loading.value = true;
  try {
    const { data, code } = await getOtherInboundOrderListApi(
    ...
    if (code === 200) {
      dataList.value = data.list as unknown as OtherInboundOrder[];
        ...
        if (valid) {
          try {
            ...
            if (row?.uid) {
              ...
              await createOtherInboundOrderApi(formData);
...
const handleView = (row: OtherInboundOrder) => {
  openDialog("查看其他入库单", row, true);
};

const handleEdit = (row: OtherInboundOrder) => {
  openDialog("编辑其他入库单", row, false);
};
...
<template>
  <div class="main">
    ...
    <el-card>
      <PureTableBar title="其他入库单列表" @refresh="fetchData">
        <template #buttons>
          <el-button
            type="primary"
            :icon="useRenderIcon(AddFill)"
            @click="openDialog('新增其他入库单')"
          >
            新增入库单
          </el-button>
        </template>
