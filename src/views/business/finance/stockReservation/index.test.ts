...
const mocks = vi.hoisted(() => ({
  getStockReservationPage: vi.fn(),
  push: vi.fn(),
  handleApiError: vi.fn()
}));

vi.mock("@/api/business/stock-ledger", () => ({
  getStockReservationPage: mocks.getStockReservationPage
}));
...
describe("business/finance/stockReservation/index", () => {
  beforeEach(() => {
    mocks.getStockReservationPage.mockReset();
    mocks.push.mockReset();
    mocks.handleApiError.mockReset();
    mocks.getStockReservationPage.mockResolvedValue({
      data: {
        count: 1,
        list: [
          {
            uid: "stock-reservation-1",
            repoId: "repo-1",
            tireId: "tire-1",
            batchNo: "BATCH-001",
  ...
  it("应加载统一预占列表并透传筛选参数", async () => {
    const wrapper = mountPage();

    await flushPromises();

    expect(mocks.getStockReservationPage).toHaveBeenCalledWith(1, {
    ...
    await inputs[0].setValue("SALE_DELIVERY_NOTE");
    await inputs[1].setValue("PICKED");
    await inputs[2].setValue("delivery-001");
    await wrapper.get("[data-test='search']").trigger("click");
    await flushPromises();

    expect(mocks.getStockReservationPage).toHaveBeenLastCalledWith(1, {
      sourceType: "SALE_DELIVERY_NOTE",
      status: "PICKED",
      sourceUid: "delivery-001"
    });
  });
  ...
  it("应支持跳转到来源工作台", async () => {
    ...
    const viewButton = wrapper
      .findAll("[data-row-uid='stock-reservation-1'] button")
