...
import {
  approveSaleContractApi,
  convertSaleContractApi,
  createSaleContractApi,
  getSaleContractsApi,
  rejectSaleContractApi,
  submitSaleContractApi,
  terminateSaleContractApi,
  type Contract,
  type ContractStatus
} from "@/api/business/sale-contract";

defineOptions({ name: "SaleContractManagement" });
...
async function loadData() {
  loading.value = true;
  try {
    const res = await getSaleContractsApi(page.current, {
...
async function createContract() {
  if (!form.title.trim() || !form.customerId.trim()) {
    message("标题和客户不能为空", { type: "warning" });
    return;
  }
  await createSaleContractApi({
...
async function submit(row: unknown) {
  const uid = contractUid(row);
  await submitSaleContractApi(uid);
...
async function approve(row: unknown) {
  const uid = contractUid(row);
  await ElMessageBox.confirm("确认通过该销售合同？", "审批");
  await approveSaleContractApi(uid);
...
async function reject(row: unknown) {
  ...
  await rejectSaleContractApi(uid, value);
...
async function terminate(row: unknown) {
  ...
  await terminateSaleContractApi(uid, value);
...
async function convertOrder(row: unknown) {
  ...
  const res = await convertSaleContractApi(uid, {
    storeId: convertForm.storeId.trim(),
    repoId: convertForm.repoId.trim()
  });
  message(`已生成草稿销售单 ${res.data?.orderUid ?? ""}`, { type: "success" });
  await loadData();
}
...
<template>
  <div class="p-4">
      ...
      <el-table-column label="操作" min-width="320" fixed="right">
        <template #default="{ row }">
          <div class="flex flex-wrap gap-1">
            ...
            <Auth
              v-if="row.status === 'PENDING_APPROVAL'"
              value="post/sale-contract/:uid/reject"
            >
            ...
            <Auth
              ...
              value="post/sale-contract/:uid/convert-order"
            >
            ...
            <Auth
              ...
              value="post/sale-contract/:uid/terminate"
            >
    ...
    <el-dialog v-model="createVisible" title="新建销售合同" width="520px">
      <el-form label-width="90px">
        ...
        <el-form-item label="商品ID">
          <el-input v-model="form.tireId" placeholder="可选 tireId" />
        </el-form-item>
