...
test.describe("产品管理", () => {
  ...
  test("页面应正确显示产品列表", async ({ authenticatedPage: page }) => {
    // 验证表格存在
  ...
  test("搜索功能应正常工作", async ({ authenticatedPage: page }) => {
    // 填写搜索条件
    await page.getByPlaceholder("产品名称").fill("轮胎");
  ...
  test("重置搜索应清空条件", async ({ authenticatedPage: page }) => {
    // 填写搜索条件
    const searchInput = page.getByPlaceholder("产品名称");
  ...
  test("点击新增按钮应显示新增对话框", async ({ authenticatedPage: page }) => {
    // 点击新增按钮
    await clickAddButton(page, "新增产品");

    // 等待对话框显示
    await waitForDialog(page);

    // 验证对话框标题
    await expect(page.locator(".el-dialog__title")).toContainText("新增产品");

    // 验证表单字段存在
    await expect(page.getByLabel("产品编码")).toBeVisible();
  ...
  test("新增产品表单验证应正常工作", async ({ authenticatedPage: page }) => {
    // 点击新增按钮
    await clickAddButton(page, "新增产品");
    await waitForDialog(page);
  ...
  test("产品分类筛选应正常工作", async ({ authenticatedPage: page }) => {
    // 查找分类下拉框（如果存在）
