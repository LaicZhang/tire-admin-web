...
test.describe("供应商管理", () => {
  ...
  test("页面应正确显示供应商列表", async ({ authenticatedPage: page }) => {
    // 验证表格存在
    ...
    // 验证新增按钮存在
    await expect(
      page.getByRole("button", { name: /新增供应商/ })
    ).toBeVisible();
  });

  test("搜索功能应正常工作", async ({ authenticatedPage: page }) => {
    // 填写搜索条件
    await page.getByPlaceholder("供应商名称").fill("测试");
  ...
  test("重置搜索应清空条件", async ({ authenticatedPage: page }) => {
    // 填写搜索条件
    const searchInput = page.getByPlaceholder("供应商名称");
  ...
  test("点击新增按钮应显示新增对话框", async ({ authenticatedPage: page }) => {
    // 点击新增按钮
    await clickAddButton(page, "新增供应商");
    ...
    // 验证表单字段存在
    await expect(page.getByLabel("供应商编码")).toBeVisible();
  ...
  test("新增供应商表单验证应正常工作", async ({ authenticatedPage: page }) => {
    // 点击新增按钮
    await clickAddButton(page, "新增供应商");
    await waitForDialog(page);
